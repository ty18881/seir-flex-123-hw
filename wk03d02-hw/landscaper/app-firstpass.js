...
const promptForUpgrade = (currPlayer) => {
    ...
    // switch statement to check wallet level and determine which tool is available for upgrade.
    ...
    if (upgradeTool.toLowerCase() === "yes" || upgradeTool.toLowerCase() === "y"){
            ...
            switch (currPlayer.tool){
                case "teeth": {
                    currPlayer.tool = toolArray[1].name;
                    currPlayer.dailyWage = toolArray[1].earnings;
                    currPlayer.wallet = 0;
                    toolArray[0].inUse = false;
                    toolArray[1].inUse = true;
                    nextThreshold = 25;
                }
